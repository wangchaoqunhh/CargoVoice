...
//        mMenuViewWidth= ScreenUtil.dp2px(context,64);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent e) {
        ...
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mPosition != INVALID_POSITION) {
                    ...
                    if (mFlingView.getChildCount() == 2) {
                        mMenuViewWidth = mFlingView.getChildAt(0).getWidth();
                        if (mDelete != null && mDelete != mFlingView.getChildAt(0)) {
                            mDelete.setBackground(null);
                        }
                        mDelete = mFlingView.getChildAt(0);
                    } else {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(xVelocity) < Math.abs(yVelocity) || Math.abs(x - mFirstX) < Math.abs(y - mFirstY)) {
                    if (mDelete != null) {
                        mDelete.setBackground(null);
                    }
                }
                ...
                case MotionEvent.ACTION_UP:
                    if (mMenuViewWidth != INVALID_CHILD_WIDTH) {
                        ...
                        if (mVelocityTracker.getXVelocity() < -SNAP_VELOCITY) {    // 向左侧滑达到侧滑最低速度，则打开
                            ...
                            mDelete.setBackground(ContextCompat.getDrawable(mContext, R.drawable.cargovoice_shape_r16dp_c005_right));
